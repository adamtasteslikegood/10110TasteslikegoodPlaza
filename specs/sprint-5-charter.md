...
Sprint 5 is Jira sprint 51 on board 169, state future (created 2026-08-20).
...
output in-world). All 8 tasks (T0–T7, PLZG-170–177) completed. The proof-of-
...
already merged, the rest To Do). The governed doc set has 27 documents — many
tier-4 summaries and research that disagree with each other and with CLAUDE.md.
The roast-me finding: 27 governed docs that disagree is more maintenance than a
...
| `D-023` | Merge commits only, squash/rebase disabled | Branch protection |
...
| T3 | Ungovern tier-4 docs + mark Sprint 2/3 charters HISTORICAL (27→≤14) | PLZG-232 | `python3 scripts/validate_specs.py` green with ≤14 docs | T0 |
...
| T5 | Sprint 5 charter and loop plan | PLZG-229 | `python3 scripts/validate_specs.py && test -f specs/sprint-5-charter.md && test -f specs/sprint-5-loop-plan.json` | T0 |
...
- **Gate B — Doc consolidation:** `python3 scripts/validate_specs.py` passes
  with ≤14 governed documents (down from 27). Governed doc count matches the registry after ungoverning.
