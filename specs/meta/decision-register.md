...
| `D-021` | Submodule tracking branch | The `claude-code-tresor` gitlink tracks **`10110TLGP/dev`** | Confirmed by the owner and by `origin/HEAD`, which points at it — it is the fork's default branch. Bumps fast-forward the pin to that branch's head. Recorded because "is the pin stale?" is unanswerable without knowing the target branch, and the answer previously lived only in someone's head. `10110TLGP/main` is divergent and is **not** a bump target; its purpose is still open — see [`spec-drivers-v0.2.5.md`](spec-drivers-v0.2.5.md) §4.7. | `PROJECT-OVERVIEW` | `LOCKED` |
