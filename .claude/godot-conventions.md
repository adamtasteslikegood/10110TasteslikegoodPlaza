...
The layout below is what exists. Extend it rather than inventing a new one:

```
project.godot           # [autoload] block registers four singletons
data/agents.json
autoload/
...
scenes/                 # each .tscn with its .gd beside it (D-025)
  ...
  bridge/ws_client.gd   # BridgeClient autoload — WebSocket client for bridge/bridge.py
tests/
...
bridge/                 # Python bridge layer (M5–M8, partially built)
  bridge.py             # WebSocket server (ws://localhost:8765), synchronous-with-timeout
  agents.py             # Agent definition loader (reads bridge/agents/*.md)
  sync.py               # Seed/upgrade tool for bridge/agents/ from submodule
  agents/               # 130 committed agent .md definitions (D-029)
```
...
The bridge layer (`bridge/`) is partially built (M5–M8). `D-005` is the hard gate: the bridge must not learn that Godot, scenes, or dialogue panels exist. If swapping the frontend for a CLI harness would require a bridge change, the boundary is broken.


## Architecture: the three Godot autoloads

All four exist and are registered in `project.godot` under `[autoload]`. New code plugs into them rather than introducing parallel state:

| Autoload | File | Role |
|---|---|---|
| `AgentRegistry` | `autoload/AgentRegistry.gd` | Loads `data/agents.json` at startup; `get_agent(id)` returns the dict |
| `GameEvents` | `autoload/GameEvents.gd` | Global signal bus: `npc_approached`, `npc_left`, `task_completed`, `floor_unlocked`, `agent_query_sent`, `agent_response_received`, `agent_query_failed` |
