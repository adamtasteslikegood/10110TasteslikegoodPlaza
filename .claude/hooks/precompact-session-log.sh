#!/usr/bin/env bash
# PreCompact hook — auto-capture session context to Confluence before compaction.
#
# Condenses the transcript, hands it to Haiku for a session summary, and
# publishes to Confluence under the Agent Session Logs index.
#
# Design rules:
#   * NEVER block compaction — every failure path exits 0.
#   * Detach and run in the background.
#   * Recursion guard via CLAUDE_PM_SESSION_LOG_ACTIVE.
...
$(cat "$DIGEST")"
  ...
  TITLED="$WORK/titled.md"
  {
    ...
    echo
    echo "> Auto-captured by the PreCompact hook (trigger: ${TRIGGER:-auto}). Summarized by Haiku."
